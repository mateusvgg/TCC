...
class L2pooling(nn.Module):
    ...
    def __init__(self, filter_size=5, stride=2, channels=None, pad_off=0):
        ...
        self.register_buffer('filter', g[None, None, :, :].repeat(
            (self.channels, 1, 1, 1)))  # type: ignore
...
class DISTS(torch.nn.Module):
    ...
    def __init__(self, load_weights: bool = True, use_pooling: bool = False):
        ...
        for x in range(5, 9):
            self.stage2.add_module(
                str(x), vgg_pretrained_features[x])  # type: ignore
        self.stage3.add_module(str(9), L2pooling(channels=128))
        for x in range(10, 16):
            self.stage3.add_module(
                str(x), vgg_pretrained_features[x])  # type: ignore
        ...
        self.register_buffer("mean", torch.tensor(
            [0.485, 0.456, 0.406]).view(1, -1, 1, 1))  # shape = [1, 3, 1, 1]
        self.register_buffer("std", torch.tensor(
            [0.229, 0.224, 0.225]).view(1, -1, 1, 1))  # shape = [1, 3, 1, 1]

        # number of feature maps at each stage
        self.chns = [3, 64, 128, 256, 512, 512]

        self.register_parameter("alpha", nn.Parameter(torch.randn(
            1, sum(self.chns), 1, 1)))  # shape = [1, 1475, 1, 1]
        self.register_parameter("beta", nn.Parameter(torch.randn(
            1, sum(self.chns), 1, 1)))  # shape = [1, 1475, 1, 1]
